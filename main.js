...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  var cards_default = () => ({
    ...
    selectCard(card) {
      ...
      if (!this.isElementInViewport(this.$refs.refCrumbs))
        this.$refs.refCrumbs.scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });
    },
    selectCrumb(crumb) {
      if (crumb == this.crumbs.slice(-1)[0])
        return;
      const truncatedCrumbs = this.crumbs.slice(
        0,
        this.crumbs.findIndex((e) => e == crumb) + 1
      );
